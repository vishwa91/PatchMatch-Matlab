...
namespace pm
{
	...
	PatchMatch::PatchMatch(float alpha, float gamma, float tau_c, float tau_g,
						   int winsize, int max_disparity)
	: alpha(alpha), gamma(gamma), tau_c(tau_c), tau_g(tau_g),
	  winsize(winsize), max_disparity(max_disparity) { }
	...
	float PatchMatch::plane_match_cost(const Plane &p, int cx, int cy, int ws, int cpv)
	{
		...
		for(int x = cx - half; x <= cx + half; ++x)
		{
			for(int y = cy - half; y <= cy + half; ++y)
			{
				...
				if(dsp < 0 || dsp > this->max_disparity)
				{
					cost += PLANE_PENALTY;
				}
				else
	...
	void PatchMatch::evaluate_planes_cost(int cpv)
	{
		...
		for(int y=0; y<rows; ++y)
			for(int x=0; x<cols; ++x)
				costs[cpv](y, x) = plane_match_cost(planes[cpv](y,x), x, y, this->winsize, cpv);
	}
	...
	void PatchMatch::spatial_propagation(int x, int y, int cpv, int iter)
	{
		...
		for(auto it = offsets.begin(); it < offsets.end(); ++it)
		{
			...
			float new_cost = plane_match_cost(p_neigb, x, y, this->winsize, cpv);
	...
	void PatchMatch::view_propagation(int x, int y, int cpv)
	{
		...
		float  new_cost = plane_match_cost(new_plane, mx, my, this->winsize, 1-cpv);
	...
	{	
		...
		while(max_dz >= end_dz)
		{
			...
			float new_cost = plane_match_cost(new_plane, x, y, this->winsize, cpv);
	...
	void PatchMatch::process_pixel(int x, int y, int cpv, int iter)
	{
		...
		// plane refinement
		plane_refinement(x, y, cpv, this->max_disparity/2, 1.0f, 0.1f);
	...
	void PatchMatch::set(const cv::Mat3b &img1, const cv::Mat3b &img2)
	{
		...
		int wmat_sizes[] = {rows, cols, this->winsize, this->winsize};
		this->weigs[0] = cv::Mat(4, wmat_sizes, CV_32F);
		this->weigs[1] = cv::Mat(4, wmat_sizes, CV_32F);
		precompute_pixels_weights(img1, this->weigs[0], this->winsize);
		precompute_pixels_weights(img2, this->weigs[1], this->winsize);
		...
		this->initialize_random_planes(this->planes[0], this->max_disparity);
		this->initialize_random_planes(this->planes[1], this->max_disparity);
	...
	void PatchMatch::postProcess()
	{
		...
		for(int x=0; x<cols; ++x)
		{
			for(int y=0; y<rows; ++y)
			{
				weighted_median_filter(x, y, disps[0], weigs[0], lft_validity, this->winsize, false);
				weighted_median_filter(x, y, disps[1], weigs[1], rgt_validity, this->winsize, false);
			}
		}
	}
